import ctypes
from ctypes import Structure, POINTER, c_double, c_int,c_char,c_float

class TReg(Structure):
    _fields_ = [("emb", c_float *128), 
                ("id",c_char *100)
                ]
...
class Tarv(Structure):
    _fields_ = [("k", c_int),
                ...
                ("cmp", ctypes.CFUNCTYPE(c_double, ctypes.c_void_p, ctypes.c_void_p, c_int)),
                ("raiz", POINTER(TNode))]

# Carregar a biblioteca C
lib = ctypes.CDLL("./kdtree.dll")
